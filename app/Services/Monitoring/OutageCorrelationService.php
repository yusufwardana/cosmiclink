...
use App\Actions\SendOutageIncidentNotification;
...
class OutageCorrelationService
{
    ...
    public function __construct(private readonly SendOutageIncidentNotification $notifications, ?int $threshold = null, ?int $windowMinutes = null)
    {
    ...
    private function correlateRouter(Router $router): void
    {
        ...
        if ($offline->count() >= $this->threshold) {
            ...
            if ($new) {
                $this->notifications->handle($incident, 'detected', User::where('tenant_id', $router->tenant_id)->firstOrFail());
            }
        ...
        if ($incident && $incident->affectedConnections()->count() > 0) {
            ...
            if ($recovered) {
                ...
                $this->notifications->handle($incident->fresh(), 'resolved', User::where('tenant_id', $router->tenant_id)->firstOrFail());
            }
        }
    }
}
