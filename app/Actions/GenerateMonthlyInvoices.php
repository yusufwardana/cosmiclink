...
class GenerateMonthlyInvoices
{
    ...
    public function handle(int $tenantId, Carbon $periodStart): int
    {
        $count = 0;
        CustomerConnection::where('tenant_id', $tenantId)
            ...
            ->get()
            ->each(function (CustomerConnection $connection) use ($periodStart, &$count): void {
                ...
                $this->generate->handle($connection, $periodStart);
                $count += $before ? 0 : 1;
            });
