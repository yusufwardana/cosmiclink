...
class SendCustomerMessage
{
    ...
    public function handle(Customer $customer, string $template, array $data, User $user, ?Invoice $invoice = null, ?CustomerConnection $connection = null, ?string $idempotencyKey = null): MessageLog
    {
        abort_unless($customer->tenant_id === $user->tenant_id, 403);
        $recipient = $this->phones->normalize($customer->phone);
        $content = $this->render($template, $data);
        $log = MessageLog::create(['tenant_id' => $customer->tenant_id, 'customer_id' => $customer->id, 'invoice_id' => $invoice?->id, 'customer_connection_id' => $connection?->id, 'channel' => 'whatsapp', 'provider' => config('messaging.provider'), 'recipient' => $recipient ?? 'invalid', 'template' => $template, 'rendered_content' => $content, 'status' => 'pending', 'attempted_at' => now(), 'idempotency_key' => $idempotencyKey]);
        if (! $recipient) {
