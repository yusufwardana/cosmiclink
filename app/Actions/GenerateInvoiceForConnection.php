...
class GenerateInvoiceForConnection
{
    ...
    public function handle(CustomerConnection $connection, Carbon $periodStart, ?User $user = null): Invoice
    {
        ...
        $invoice = DB::transaction(function () use ($connection, $start, $end) {
        ...
        if ($user && $connection->customer->tenant_id === $user->tenant_id) {
            $this->messenger->handle($connection->customer, 'invoice_created', ['customer_name' => $connection->customer->name, 'invoice_number' => $invoice->invoice_number, 'amount' => $invoice->total, 'period' => $start->format('F Y')], $user, $invoice, $connection);
        }
