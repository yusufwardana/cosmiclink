...
## Task 4.2B Slice 3 Evidence — Platform Master-Key Providers — 2026-09-20

- Windows architecture: added a build-tagged production `MasterKeyProvider` using Windows DPAPI through `golang.org/x/sys/windows`. The provider owns only the installation master-key lifecycle; Slice 2 SQLite/AES-GCM store code and provider wiring were not redesigned.
- DPAPI scope: selected current-user DPAPI scope (`CryptProtectData`/`CryptUnprotectData` without `CRYPTPROTECT_LOCAL_MACHINE`) for the intended dedicated Agent service identity. The protected blob is usable by that service identity and is not claimed to resist local Administrator/SYSTEM compromise. No machine-wide DPAPI scope was selected.
- Installation binding: installation identity is authenticated as DPAPI optional entropy and is also recorded as non-secret key-file metadata. A copied blob with a different installation identity fails before key release; changing metadata to another identity cannot unwrap the original entropy-bound blob.
- Key file format: versioned JSON contains only `format_version`, `installation_id`, `protector`, base64 DPAPI-protected key bytes, and `created_at`. Unknown fields, trailing JSON, malformed/truncated content, unsupported versions, invalid identities, and missing metadata fail closed. The file is written through a temporary file with `0600` best-effort permissions and atomic rename; parent directories use `0700` best effort.
- Explicit initialization: `InitializeProductionMasterKey` generates exactly 32 bytes with `crypto/rand`, protects them with DPAPI, persists the protected envelope, then unwraps it to verify initialization. Existing files are never overwritten. `MasterKey` never generates, repairs, wipes SQLite, or falls back to environment/test/zero keys.
- Key loss/corruption: missing, unreadable, malformed, truncated, unsupported-version, tampered, DPAPI-unwrappable, and wrong-installation material fails closed. No automatic regeneration or database modification occurs.
- Linux/non-Windows boundary: production provider construction and initialization return deterministic `ErrMasterKeyProviderUnsupported`. No fake Linux OS-security guarantee, plaintext file-key provider, environment fallback, or in-memory production fallback was added. Linux target build succeeds from the current Windows environment; non-Windows test binaries were not executed because they cannot run on Windows.
- Test provider boundary: existing deterministic in-memory providers remain test-local and are not selected by production constructors. Fake mutation provider tests remain independent of credential stores and master keys.
- Secret safety: Windows tests cover 32-byte unwrap, distinct random keys across installations, second initialization refusal, missing-key non-regeneration, malformed/truncated/unsupported/tampered files, installation mismatch, sentinel absence from protected files, correct-key SQLite reopen, and wrong-key credential failure. Master-key material is not included in logs, errors, JSON, HTTP, job, or Core structures.
- Dependency: promoted existing transitive `golang.org/x/sys` to direct `v0.22.0` for the Windows DPAPI syscall wrappers. No unrelated dependency upgrades; `go mod tidy` and `go mod verify` passed.
- Regression result: native Windows focused credential-provider tests passed; full Go `go test ./...`, `go vet ./...`, `go build ./...`, `gofmt`, and `go mod verify` passed. Linux cross-build passed. `go test -race ./internal/credentials` remains unavailable because Windows CGO is disabled; cross-compiled non-Windows tests cannot execute on this Windows host.
- Laravel/Core boundary: no Laravel migration, Core master-key storage, Core operator-secret storage, master-key endpoint, FutureMutationContext key field, or network request key field was added. No persistent credential store or master-key provider was wired into mutation or RouterOS providers.
- Zero-activity evidence: no real operator credential created; real MikroTik connections: 0; real RouterOS writes: 0; RouterOS configuration changed: no; RouterOS permissions changed: no; `NETWORK_MUTATIONS_ENABLED` remains `false`; `NETWORK_MUTATION_PROVIDER` remains `fake`.
- Commit boundary: `Phase 6I Task 4.2B add platform master key providers`.

## Task 4.2B Slice 4 Evidence — Agent Credential Resolver Integration — 2026-09-20

- Agent boundary: added `AgentCredentialResolver` under `network-engine/internal/agent`. It implements the existing hardened `credentials.Resolver` contract and provides a separate `ResolveJob` adapter for a strict Agent reference-only job shape.
- Authoritative installation identity: the resolver is constructed with one validated local `InstallationIdentity`; incoming job installation IDs are required and must match exactly. The request cannot override local identity.
- Exact local-store resolution: Agent jobs carry only tenant, router, agent, installation, credential reference, purpose, and positive version fields. Resolution converts those fields to one exact `credentials.Reference` and calls only the encrypted SQLite store; no latest/highest/version/purpose fallback exists.
- Isolation: store matching remains exact for tenant, router, agent, installation, credential reference, purpose, version, and ACTIVE status. Observer/operator, version, retired, revoked, unknown-status, missing, malformed, wrong-scope, wrong-key, corrupt-payload, and installation failures remain fail-closed through Slice 2/3 error vocabulary.
- Reference-only contract: `CredentialResolutionJob` uses strict JSON decoding with `DisallowUnknownFields`; unknown secret-bearing fields are rejected. No username, password, secret, encrypted payload, master key, protected blob, or connection object is present in the contract.
- Synthetic consumer: `ConsumeResolvedCredential` accepts a resolved credential only for a narrow in-process test seam and returns safe evidence containing resolved/purpose/version/reference fingerprint only. Username and secret are not returned or serialized.
- Secret lifetime: resolved material is passed directly to the synthetic consumer and is not attached to legacy Agent jobs, Agent result payloads, logs, errors, JSON, Core requests, or provider state. Resolver `Close` releases the store and its in-process key copy; memory erasure is not claimed beyond existing best-effort behavior.
- Legacy and provider isolation: legacy read-only discovery jobs remain unchanged, including their existing discovery connection payload. `NewWithCredentialResolver` is opt-in and does not invoke discovery, mutation, RouterOS, go-routeros, or any network socket. Fake mutation-provider construction remains independent of resolver/store/master-key construction; RouterOS mutation selection remains fail-closed.
- Production construction: `NewProductionCredentialResolver` composes the platform master-key provider and existing store only after the store file already exists. It does not initialize missing keys, create missing stores, enroll credentials, use environment fallback, or wire into a provider. Non-Windows construction remains unsupported through Slice 3's fail-closed provider boundary.
- Core/Laravel safety: no Laravel migration, public credential API, Core secret storage, or new secret field was added. Existing FutureMutationContext and Go network.Request remain reference-only.
- Verification: focused Agent/credentials/provider tests passed; full Go `go test ./...`, `go vet ./...`, `go build ./...`, `gofmt`, `go mod verify`, Linux `GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build ./...`, full Laravel suite, Pint, PHP syntax checks, and `git diff --check` passed. `go test -race ./internal/agent ./internal/credentials` was attempted and is unavailable because Windows CGO is disabled.
- Zero-activity evidence: synthetic sentinel credentials only; real operator credential created: no; real MikroTik connections: 0; real RouterOS writes: 0; RouterOS configuration or permissions changed: no; no network socket or mutation transport activity; `NETWORK_MUTATIONS_ENABLED` remains `false`; `NETWORK_MUTATION_PROVIDER` remains `fake`.

## Task 5A Evidence — Agent Installation Foundation — 2026-09-20

- Starting baseline: required HEAD `c8c1db5aed1cd468a8e88b1fa4ef1022dbea6804` on branch `phase-6i-controlled-routeros-operations`; the working tree was clean before implementation.
- Authoritative identity: `NetworkAgent.identifier` remains the only source of `agent_ref`, received through the authenticated heartbeat and persisted by 5A.0 in `agent-bootstrap.json`. Installation initialization reads and verifies that state; it never generates, derives, accepts operator input for, or rebinds `agent_ref`.
- Installation artifact: added strict version-1 `installation.json` containing only `format_version`, UUIDv4 `installation_id`, authoritative `agent_ref`, and `created_at`. It rejects malformed/truncated JSON, unknown fields, trailing data, missing/invalid fields, unsupported versions, and bootstrap/install reference mismatch. Writes are atomic with `0600` best-effort file permissions and `0700` parent-directory creation.
- Installation identity: `installation_id` is generated once with `crypto/rand` as UUIDv4, is independent from `agent_ref`, and is preserved across repeated initialization. A completed installation returns `AGENT_ALREADY_INITIALIZED`-equivalent behavior and does not replace metadata or key material.
- Initialization/runtime boundary: `InitializeInstallation` requires an existing authoritative bootstrap, explicitly creates and verifies the installation-scoped protected master key, persists installation metadata, then reopens/verifies both artifacts. `OpenInstallation` only validates existing state and unwraps the key; it never creates or repairs installation, key, or credential-store artifacts. Failed initialization removes only artifacts created by that attempt and preserves `agent-bootstrap.json`.
- Master key: existing Windows current-user DPAPI provider remains authoritative. `installation_id` is the DPAPI entropy/binding context; missing, corrupt, wrong-installation, or unavailable protected key material fails closed; no automatic regeneration, plaintext fallback, environment fallback, or machine-wide DPAPI was added. Non-Windows provider behavior remains unsupported/fail closed.
- Credential store lifecycle: added explicit `InitializeStore` and `OpenExistingStore` APIs. Production resolver construction now uses `OpenExistingStore`, and Agent-level `InitializeCredentialStore`/`OpenCredentialStore` validate an initialized Agent installation before accessing `credentials.db`. Open-existing never creates a missing database or schema.
- Store marker/version: initialized SQLite stores now contain non-secret `store_metadata` marker `cosmiclink.credentials` at schema version `1`; empty SQLite files, unrelated databases, partial schemas, corrupt databases, and unsupported marker versions fail closed. Existing `OpenStore` remains only as a compatibility helper for synthetic tests and preserves its legacy schema-error wording.
- Version monotonicity: added transactional `Store.NextVersion` over the exact tenant/router/agent/installation/credential/purpose scope. It computes `max(version)+1`, counts retired/revoked history, isolates scopes, and proves `v1,v3 -> v4` and empty scope -> `1` in tests. No enrollment or operator-selected version path was added.
- Secret safety: installation metadata and schema marker contain no token, master key, protected blob, username, or credential secret. Existing encrypted-store and DPAPI tests continue to reject sentinel leakage; no secrets are added to logs, errors, transport contracts, or Core/Laravel structures.
